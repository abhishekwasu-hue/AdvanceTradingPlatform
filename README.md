...
This phase delivers the **strategy & execution core**: indicators, inbuilt auto-executable
multi-timeframe and indicator-based intraday scalping strategies, a risk engine, a paper
execution router, a backtest engine, and a FastAPI service exposing all of it.
...
plain-English reasons behind it — and every order (paper today, live once a broker adapter is
wired in) passes through the Risk Engine before execution. Nothing here places a live order:
`ExecutionMode.LIVE` is intentionally blocked until a real broker integration exists.
